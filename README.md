...
这是一个**最基础版本的规划器**。它的意义不在于"好用"，而在于**确定规划器的输入和输出应该是什么样子**，为后续更高级、更可用的规划器提供一个基础。

具体功能：从专家路径派生多个候选路径，这些候选路径用于生成多组预测图像，辅助打分器进行选择。这是一个比较粗糙的初步实现。
...
将来我们设计的整个系统的运作方式应该是**走一步看一步**的，详细步骤参考飞书上的流程图：每步只考虑下一步的几个候选动作，执行完再观测、再决策。

ANWM 的方式更像是"视频生成"：规划器一次性给出长序列，世界模型自回归生成一长串视频帧，最后拿最后一帧跟目标图片比相似度。
...
|   **打分依据**   |              最后生成的帧与目标图片的**相似度**              |                   世界模型预测的未来帧质量（具体待定）                    |
...
- 视觉模型拿着短序列生成未来帧
- 未来帧输入打分器
- 规划器根据它给出的序列和视觉模型返回的帧进行详细思考，给出最优轨迹
