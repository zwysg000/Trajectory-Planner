...
后续将参考近期流行的**基于大模型驱动的规划器**，进行真实可用规划器的设计。主要参考以下两篇文章：

- **LookasideVLN**（CVPR 2026）— Direction-Aware Aerial Vision-and-Language Navigation。面向无人机航拍的视觉语言导航，通过地标知识库与前瞻路径规划提升指令理解能力，解决城市复杂环境下的导航问题。 [arXiv: 2604.17190](https://arxiv.org/abs/2604.17190)
- **Uni-LaViRA** — Language-Vision-Robot Actions Translation for Unified Embodied Navigation。将导航建模为 **language → vision → robot action** 三层翻译的 **training-free** 框架，直接利用预训练多模态大模型进行语言规划、视觉落地的动作决策，已发布开源代码。 [arXiv: 2605.27582](https://arxiv.org/abs/2605.27582) | [GitHub](https://github.com/NJU-R-L-Group-Embodied-Lab/uni-lavira-code)

整体技术路线将基于**视觉语言大模型驱动**，后续规划器不依赖专家轨迹，直接根据 Instruction 和当前画面输出候选动作。
...
|   **打分依据**   |              最后生成的帧与目标图片的**相似度**              |              世界模型预测的未来状态和规划器预期的匹配度（具体待定）               |
|  **规划器角色**  |                      一次性生成完整路径                      |                         每步生成短动作候选，由打分器筛选                          |

我们方案的关键点：

1. **规划器只输出很短的短动作序列**（告诉无人机下一步做什么），而不是一次性规划完整路径
2. **世界模型对这些短序列进行预测**（生成未来帧等状态信息），而不是直接生成完整视频
3. **打分器对预测结果进行评分**：结合规划器给出的候选序列和世界模型返回的预测结果，选出最优动作
...
- 打分器需要判断世界模型预测的未来状态**是否符合规划器给出的动作预期**
